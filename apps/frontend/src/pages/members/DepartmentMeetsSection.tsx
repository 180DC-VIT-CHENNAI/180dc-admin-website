  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {meets.length === 0 && <div style={{ padding: "2rem", textAlign: "center", color: "var(--text-tertiary)", border: "1px dashed var(--outline-variant)", borderRadius: 12 }}>No department meets scheduled.</div>}
        {meets.map((m) => (
          <div key={m.id} style={{ 
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
               ...
               <div>
                  ...
                  <div style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 2, display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{ color: "var(--primary-green)", fontWeight: 600 }}>{m.department_name}</span>
                  </div>
               </div>
            </div>
      ...
      {isLead && (
        <div style={{ marginTop: "1rem", padding: "1rem", background: "var(--surface-container-low)", borderRadius: 16, display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "var(--text-tertiary)" }}>SCHEDULE FOR {userDeptName?.toUpperCase()}</div>
